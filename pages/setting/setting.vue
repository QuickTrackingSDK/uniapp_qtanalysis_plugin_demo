...
<script>
	...
	// #endif
	import QT from '../../utils/sdkAdaptor.js'
	export default {
		data() {
			return {
				title: 'textarea',
		...
		methods: {
				...
				uni.showModal({
					...
					success(res) {
						if(res && res.confirm) {
							...
							QT.registerGlobalProperties(that.gp);
						} else {
			...
			getGlobalProperties() {
				...
				// })
				// // #endif
				var curGP = QT.getGlobalProperties();
				console.log('yz----curGP', curGP);
				uni.showModal({
					title: '当前全局属性',
					content: JSON.stringify(curGP || {}),
				...
				uni.showModal({
					...
					success(res) {
						if(res && res.confirm) {
							...
							QT.sendEvent('$$_user_profile', that.up, 'OTHER')
						} else {
			...
			setUserIdAndOrganization() {
				...
				uni.showModal({
					...
					success(res) {
						if(res && res.confirm) {
							QT.setUserId(that.puid, that.organization);
						} else {
